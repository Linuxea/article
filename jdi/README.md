...
Java调试接口 API 是 Java 提供的一组接口，用于实现调试器的前端。JDI 是 JPDA 的最高层。
...
今天我们会通过编写一些案例来实现编程式的调试，通过这些步骤来熟悉 `JDI`。并且会提到相关的远程调试技术。
...
目前为止 debugger 还没有真正的实现编程式调试，因为我们还没有完成调试`类的准备工作`与`断点设置`。
